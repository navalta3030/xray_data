...
class SickNessPipeLine(XrayTF):
    def __init__(self, ImageSize, LabelQuota, BatchSize, CsvFile, Epoch):
        """
        Main purpose of this one is a pipeline for each sickness

        @param - ImageSize  : Integer       - pixel size of the image
        @param - LabelQuota : Integer       - Minimum number required for labels to be admitted to the whole training session.
    ...
    def _pre_process(self):
        ...
        self.prune_based_on_quota(LABEL_QUOTA)
        # make all labels equal based on the lowest label.
        self.balance_all_labels()
    ...
    def _main_process(self):
        ...
        if self.model_exist("All"):
            model = self.model_exist("All")
    ...
    def download_images(self):
        ...
        """
        shutil.rmtree(LOCAL_IMAGES_LOCATION)
